...
export default function Home() {
  ...
  const [pipeline, setPipeline] = useState({ gemini: false, worker: false });
  ...
  useEffect(() => {
    ...
    fetch("/api/generate-3d")
      .then((r) => r.json())
      .then((j) => setPipeline((p) => ({ ...p, worker: !!j.configured })))
      .catch(() => {});
    ...
    try {
      ...
      pollRef.current = setInterval(async () => {
        try {
          ...
          if (status.status === "texturing") setGen({ state: "texturing", detail: `Task ${taskId}` });
